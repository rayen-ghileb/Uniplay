  ...
  const createGameMutation = useMutation({
    ...
    onSuccess: (game) => {
      ...
      navigate(`/games/${game.id}`, {
        state: {
          feedback: game.is_public
            ? "Terrain réservé avec succès. Les joueurs intéressés rejoindront votre partie."
            : "Terrain réservé avec succès. Invitez des joueurs à votre partie.",
        },
      });
    },
    onError: (err) => {
      ...
      if (typeof errData === "object") {
        setErrorMsg(Object.values(errData).flat().join(" ") || "Une erreur est survenue.");
      } else if (typeof errData === "string") {
        setErrorMsg(errData);
      } else {
        setErrorMsg("Erreur lors de la création du jeu.");
      }
    },
  });
          ...
          <div className="relative flex items-start justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-xs font-medium text-white/65">
                Choisissez un créneau, vous inviterez vos joueurs juste après.
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="rounded-2xl border border-gray-200 bg-fog/40 p-4 sm:p-5">
            <div className="mb-4">
              ...
              <p className="mt-1 text-xs font-medium text-steel">
                Un jeu public peut être rejoint librement, un jeu privé uniquement sur invitation.
              </p>
            </div>
            <div className="grid gap-3 sm:grid-cols-2">
              <button
                ...
                className={`rounded-xl border p-4 text-left transition-all ${
                  isPublic
                    ? "border-crimson bg-white shadow-md ring-2 ring-crimson/20"
                    : "border-gray-200 bg-white hover:border-crimson/40"
                }`}
              >
                <div className="text-sm font-bold text-ink">🌍 Public</div>
                <p className="mt-1 text-xs font-medium text-steel">
                  Visible dans la liste des jeux, tout le monde peut rejoindre.
                </p>
              </button>
              <button
                type="button"
              ...
              >
                <div className="text-sm font-bold text-ink">🔒 Privé</div>
                <p className="mt-1 text-xs font-medium text-steel">
